...
// Only the filesystem probes and the one write helper are faked.
// `sanitizePath` (reached through PATH_TO_SPACE_SEGMENTS) and the rest stay
// REAL — a stubbed path sanitiser would make the traversal case below pass for
// the wrong reason. `isPathExists` / `isPathIsDir` are what
// `SpaceGuard.checkPermissions` consults to resolve a PUT to MODIFY.
...
const SPACE_BASE = {
  realPath: '/data/test.drawio',
  relativeUrl: 'test.drawio',
  enabled: true,
  inTrashRepository: false,
  quotaIsExceeded: false,
  dbFile: { path: 'test.drawio' }
}
...
describe('CustomDiagramsService', () => {
  ...
  describe('save', () => {
    ...
    // between. The write must land on the live path itself. This case replaces
    // the two tmpfile/rename cases that pinned the old behaviour.
  ...
  describe('createNew', () => {
    /* `mkFile` throws FileError / LockConflict, and BOTH extend Error rather
       ...
       This table subsumes develop's two single-status cases ("translates
       mkFile's FileError into its own status" / "translates a LockConflict into
       423"): it pins the same two statuses AND the message each carries, plus
       that nothing was seeded and nothing announced. */
    it.each([
      [
