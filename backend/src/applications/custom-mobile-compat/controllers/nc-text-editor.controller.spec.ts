...
describe(NcTextEditorController.name, () => {
  ...
  describe('GET /custom-mobile-compat/text-editor/codemirror.bundle.js', () => {
    it('returns 404 when the bundle has not been built', async () => {
      // The bundle is a separate build artifact — until generated, the editor
      // page falls back to <textarea>. This 404 IS the fallback signal.
      const r = makeRes()
      await expect(controller.bundle(r.res)).rejects.toMatchObject({ status: 404 })
    })
  })
})
