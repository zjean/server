...
// Exported because `uploadId` also reaches the filesystem OUTSIDE this service:
// the assembly step builds a sibling tmp path from it in nc-uploads.controller.
// That call site used the raw route param and was a traversal primitive — see
// the header note on assembleAndMove. Any new code that interpolates an
// upload id into a path must go through this.
export function sanitizeUploadId(segment: string): string {
...
function sanitize(segment: string): string {
  ...
  //
  // The two rules are not redundant: stripping separators alone still leaves
  // `..` able to climb once the surrounding string supplies the slash (e.g.
  // `<real>.uploading.` + `..`), and stripping dots alone leaves `%2F`-decoded
  // separators intact. find-my-way decodes `%2F` and `%2E` in path params, so
  // both forms arrive here already decoded.
