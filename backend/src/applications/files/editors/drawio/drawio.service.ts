...
export class DrawioService {
  ...
  async load(user: UserModel, path: string): Promise<LoadDiagramResponse> {
    ...
    if (!(await isPathExists(space.realPath))) throw new HttpException('file not found on disk', HttpStatus.NOT_FOUND)
    ...
    return {
      ...
      editorUrl: configuration.applications.files.drawio.url
    }
  }
  ...
  async createNew(user: UserModel, dto: NewDiagramDto): Promise<{ path: string }> {
    ...
    await writeFile(space.realPath, EMPTY_DIAGRAM_XML, 'utf-8')
