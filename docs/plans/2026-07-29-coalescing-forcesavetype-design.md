...
> described below (§3, :87). `saveKind` is now `'interactive' | 'automatic' | 'human'`, not the two-value type at
> :84; "unclassifiable" (§2.2's absent-field / status 2 / status 3 rows) is classified `'interactive'`, which is
> specifically **not** `'human'` (:62). And `maxVersionsPerFile` (:130) no longer exists — the per-file FIFO cap it
