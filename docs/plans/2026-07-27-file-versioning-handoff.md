...
- Two timestamps per row, and they are not interchangeable: `mtime` is when the revision's own bytes were written, `createdAt` is when the overwrite retired it. They can be months apart. C2 has to label rows with a deliberate choice of one.
