...
export class SpaceFilesComponent implements OnInit, OnDestroy {
  ...
  protected openEntry(file: FileProps): void {
    ...
    if (isTextEditable(file)) {
      // isWriteable=true is optimistic — the server enforces permissions when we
      // attempt the LOCK request, and the dialog falls back to read-only on 403.
      // TODO: derive from real space permissions once v2 surfaces them.
  ...
  private buildFullPath(alias: string, file: FileProps): string {
    const segs = this.pathSegments().map((s) => s.path)
    return [SPACE_REPOSITORY.FILES, alias, ...segs, file.name].join('/')
  }
